...
void KMeans::printResult()
{
    ...
    for (int i = 0; i < numClusters; i++)
    {
        ...
        if (cluster[i].numPoint == 0) { cout << "(no data in this cluster!)\n"; continue; }
...
void KMeans::checkAccuracy()
{
    ...
    for (auto point : dataPoints)
    {
        checkFile << "point label is: " << point.label << " - point cluster is: " << point.clusterId << " - this cluster label is: " << cluster[point.clusterId].label << endl;
    }
    checkFile.close();
    ...
    {
        cout << "Cluster " << i << " true label is: " << _cluster.label << endl;
        i++;
    }
    int cnt = 0;
